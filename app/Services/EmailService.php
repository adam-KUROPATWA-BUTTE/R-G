...
class EmailService
{
    ...
    public function __construct()
    {
        ...
        $configPath = __DIR__ . '/../../config/config_email.php';
        if (file_exists($configPath)) {
            require_once $configPath;
        }

        $this->companyName = defined('COMPANY_NAME') ? COMPANY_NAME : 'R&G';
        $this->companyEmail = defined('COMPANY_EMAIL') ? COMPANY_EMAIL : 'contact@example.com';
        $this->companyPhone = defined('COMPANY_PHONE') ? COMPANY_PHONE : '';
        $this->siteUrl = defined('SITE_URL') ? SITE_URL : '';
        $this->emailFrom = defined('EMAIL_FROM_ADDRESS') ? EMAIL_FROM_ADDRESS : $this->companyEmail;
        $this->emailFromName = defined('EMAIL_FROM_NAME') ? EMAIL_FROM_NAME : $this->companyName;
        $this->emailAdmin = defined('EMAIL_ADMIN') ? EMAIL_ADMIN : $this->companyEmail;
    }
    ...
    public function sendOrderConfirmation(array $order, array $items): bool
    {
        ...
        $subject = "Confirmation de commande #" . $order['id'] . " - " . $this->companyName;
        
        $html = $this->generateOrderEmailHtml($order, $items);
        
        $headers = [
            'MIME-Version: 1.0',
            'Content-type: text/html; charset=UTF-8',
            'From: ' . $this->emailFromName . ' <' . $this->emailFrom . '>',
            'Reply-To: ' . $this->companyEmail,
     ...
     */
    public function sendAdminOrderNotification(array $order): bool
    {
        $to = $this->emailAdmin;
        $subject = "🔔 Nouvelle commande #" . $order['id'] . " reçue";
        
        $html = $this->generateAdminNotificationHtml($order);
     ...
     */
    private function generateOrderEmailHtml(array $order, array $items): string
    {
...
<body style='margin: 0; padding: 0; font-family: Arial, sans-serif; background-color: #f3f4f6;'>
                ...
                <table width='600' cellpadding='0' cellspacing='0' style='background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1);'>
                    <tr>
                        <td style='background: linear-gradient(135deg, #1e3a8a 0%, #3b82f6 100%); padding: 40px 30px; text-align: center;'>
                            <h1 style='color: #ffffff; margin: 0; font-size: 28px;'>" . $this->companyName . "</h1>
                    ...
                    <tr>
                        <td style='background-color: #f9fafb; padding: 30px; text-align: center; border-top: 1px solid #e5e7eb;'>
                            ...
                            <p style='margin: 0 0 10px 0;'>
                                📧 <a href='mailto:" . $this->companyEmail . "' style='color: #3b82f6; text-decoration: none;'>" . $this->companyEmail . "</a><br>
                                📞 " . $this->companyPhone . "
                            </p>
                            <p style='margin: 20px 0 0 0; color: #9ca3af; font-size: 12px;'>
                                © " . date('Y') . " " . $this->companyName . ". Tous droits réservés.<br>
                                <a href='" . $this->siteUrl . "' style='color: #3b82f6; text-decoration: none;'>" . $this->siteUrl . "</a>
                            </p>
                        </td>
                    </tr>
                </table>
     ...
     */
    private function generateAdminNotificationHtml(array $order): string
    {
        $orderNumber = (int)$order['id'];
        $total = number_format((float)$order['total'], 2, ',', ' ');
